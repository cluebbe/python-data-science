...
| [Gradient Boosting](GRADIENT_BOOSTING_TUTORIAL.md) | Train a gradient boosting regressor on the diabetes dataset. Covers boosting vs. bagging, hand-building the residual-fitting loop from scratch, the learning rate / n_estimators tradeoff, staged predictions, and overfitting via too many boosting rounds. | [.md](GRADIENT_BOOSTING_TUTORIAL.md) · [.py](gradient_boosting_tutorial.py) |
...
| [K-Nearest Neighbors + Google Maps](knn-google-maps/KNN_GOOGLE_MAPS_TUTORIAL.md) | Build a Flask + Google Maps app that uses KNN for geographic nearest-neighbor search and location classification. Covers the haversine distance metric, `NearestNeighbors` vs. `KNeighborsClassifier`, `predict_proba` confidence scores, and wiring a Python backend to a JS frontend. | [.md](knn-google-maps/KNN_GOOGLE_MAPS_TUTORIAL.md) · [.py](knn-google-maps/app.py) · [.html](knn-google-maps/templates/index.html) |
